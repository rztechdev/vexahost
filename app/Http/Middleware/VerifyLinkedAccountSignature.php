 ...
 *
 * Timestamp menolak pemutaran ulang, HMAC atas timestamp + isi body
 * membuktikan asalnya WA Gateway. Rahasia yang kosong mematikan endpoint sama
 * sekali (503), bukan membukanya.
 */
class VerifyLinkedAccountSignature
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (! hash_equals(hash_hmac('sha256', $waktu.'.'.$request->getContent(), $secret), $tandaTangan)) {
            // Tanda tangan salah berulang ke endpoint yang bisa mengganti kata
            // sandi adalah tanda seseorang sedang mencoba.
