...
/**
 * Mengirim perubahan akun ke VexaHost WA Gateway — sisi keluar akun tertaut.
 *
 ...
 *
 * Tidak pernah melempar galat ke pemanggilnya. Pendaftaran, checkout, dan
 * penggantian kata sandi di sini tidak boleh gagal hanya karena WA Gateway
 * sedang tidak menjawab — yang gagal ditandai tertunda dan diulang
 * `akun-tertaut:kirim` tiap menit.
 *
 * Protokolnya harus sama persis dengan salinannya di repo vexahost-wa
 * (docs/AKUN_TERTAUT.md di sana).
 */
class LinkedAccountSync
{
    ...
    public function push(User $user, string $mode = self::MODE_SYNC): array
    {
        ...
        try {
            ...
            Log::warning('Akun tertaut: WA Gateway tidak terjangkau, diulang nanti.', [
                'user_id' => $user->id,
                'galat' => $e->getMessage(),
            ]);
        ...
        if ($jawaban->clientError() && ! in_array($jawaban->status(), [401, 429], true)) {
            Log::error('Akun tertaut: WA Gateway menolak perubahan akun.', [
                'user_id' => $user->id,
        ...
        Log::warning('Akun tertaut: WA Gateway galat, diulang nanti.', [
            'user_id' => $user->id,
            'status' => $jawaban->status(),
        ]);
